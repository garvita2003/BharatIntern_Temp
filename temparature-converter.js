...
//add event listner that on input call particular function
...
//conversion if input provided in celcius input box
function celciusOnInput() {
   let C = celsius.value;
   if (C !== "") {
      C = parseInt(C);
      farenheit.value = ((C * 1.8) + 32).toFixed(1); //decimal places
...
//conversion if input provided in farenheit input box
...
//conversion if input provided in kelvin input box
...
//conversion if input provided in rankine input box
...
//conversion if input provided in reaumur input box
...
//additional info attached to buttons and add event listner of onclick and call on celius func and calculate rest values
